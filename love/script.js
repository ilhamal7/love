...
(function () {
    ...
    window.addEventListener("load", function () {
        ...
        render();
        // Tambahin ini buat munculin teks di tengah
        setInterval(() => {
            const text = document.getElementById("loveText");
            text.innerHTML = ""; // Bersihin teks sebelumnya

            let j = 0;
            const message = "Semangat💓"; // Pesan tunggal yang diinginkan

            function typeWriter() {
                if (j < message.length) {
                    text.innerHTML += message.charAt(j);
                    j++;
                    setTimeout(typeWriter, 100); // Tulis satu per satu
                }
            }

            typeWriter(); // Panggil fungsi ketik

            setTimeout(() => {
                text.style.opacity = 1; // Tampilkan teks setelah selesai ketik
            }, 100); // Tampil setelah efek ketikan selesai

            setTimeout(() => {
                text.style.opacity = 0; // Sembunyi setelah 8 detik
            }, 8000); // tampil selama 8 detik
